        ...
        using (FileStream fs = new FileStream(BinaryDataMgr.DATA_BINARY_PATH + table.TableName + ".txt", FileMode.OpenOrCreate, FileAccess.Write))
        {
            ...
            for (int i = BEGIN_INDEX; i < table.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < table.Columns.Count; j++)
                {
                    ...
                    {
                        case "int"://如果没有填写数据，就是默认值
                            fs.Write(BitConverter.GetBytes(row[j].ToString() == string.Empty ? 0 : int.Parse(row[j].ToString())), 0, 4);
                            break;
                        case "short":
                            fs.Write(BitConverter.GetBytes(row[j].ToString() == string.Empty ? 0 : short.Parse(row[j].ToString())), 0, 2);
                            break;
                        case "long":
                            fs.Write(BitConverter.GetBytes(row[j].ToString() == string.Empty ? 0 : long.Parse(row[j].ToString())), 0, 8);
                            break;
                        case "byte":
                            fs.Write(new byte[] { row[j].ToString() == string.Empty ? (byte)0 : byte.Parse(row[j].ToString()) }, 0, 1);
                            break;
                        case "uint":
                            fs.Write(BitConverter.GetBytes(row[j].ToString() == string.Empty ? 0 : uint.Parse(row[j].ToString())), 0, 4);
                            break;
                        case "ushort":
                            fs.Write(BitConverter.GetBytes(row[j].ToString() == string.Empty ? 0 : ushort.Parse(row[j].ToString())), 0, 2);
                            break;
                        case "ulong":
                            fs.Write(BitConverter.GetBytes(row[j].ToString() == string.Empty ? 0 : ulong.Parse(row[j].ToString())), 0, 8);
                            break;
                        case "float":
                            fs.Write(BitConverter.GetBytes(row[j].ToString() == string.Empty ? 0f : float.Parse(row[j].ToString())), 0, 4);
                            break;
                        case "double":
                            fs.Write(BitConverter.GetBytes(row[j].ToString() == string.Empty ? 0 : double.Parse(row[j].ToString())), 0, 8);
                            break;
                        case "bool":
                            fs.Write(BitConverter.GetBytes(row[j].ToString() == string.Empty ? false : bool.Parse(row[j].ToString())), 0, 1);
                            break;
                        case "string":
